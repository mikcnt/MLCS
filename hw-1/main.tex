...
To prove that the substructure $\mathfrak{B}$ is not elementary, we can consider the formula expressing the existence of the root (e.g., in particular we can consider the cubic root to avoid sign problems):
\[
    \forall x \exists y (y \times y \times y = x).
\]
This is clearly true for $\mathcal{R}$, but it is not for $\mathcal{Q}$. Therefore $\mathfrak{B}$ is not an elementary substructure.
\end{proof}
