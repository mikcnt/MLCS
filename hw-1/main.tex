...
\]
This formula is satisfied in ($\mathbb{Q}$, $+$, $\times$, $0$, $1$, $<$), but not in ($\mathbb{N}$ +, $\times$, $0$, $1$, $<$). As for the hint, we know that two structures are isomorphic if and only if they satisfy the same sentences, therefore we can conclude that these two are not.
...
\proofpart{}
Finally, to prove that ($\mathbb{Q}$, $<$) and ($\mathbb{R}$, $<$), we can trivially conclude that those structures are not isomorphic for cardinality reasons.
\end{proof}
\end{document}
