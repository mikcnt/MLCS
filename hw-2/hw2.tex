...
\begin{proof}
	...
	Intuitively, in order to build such structures, one could build $\mathfrak{G}_1$ and $\mathfrak{G}_2$ such that their number of nodes is very big compared to the fixed number of moves for the game. For the first structure we can consider a (very big) cycle, and for the second we can consider a structure made by two disjoint connected graphs.
	
	In particular, we can generalize the first two cases of the exercise; therefore, for each $k \in \mathbb{N}$, we can consider $\mathfrak{G}_1$ to be the cycle of length $4k$, and $\mathfrak{G}_2$ to be the disjoint structure composed by two cycles of length $2k$.
