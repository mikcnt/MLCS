...
To prove that the acyclicity query is not expressible in the language of graphs over finite graphs, we must show that, for every $k$, there are two finite structures $\mathfrak{A}$ and $\mathfrak{B}$ such that $\mathfrak{A}$ contain a cycle and $\mathfrak{B}$ does not, and such that Discriminator wins the game $G_k$ on ($\mathfrak{A}$, $\mathfrak{B}$).

Let's assume now that $\mathfrak{A}$ is a cycle of length $k + 2$. This obviously contains a cycle, and therefore we can affirm that the aciclicity query is respected by it.
\input{structures/ex1_5_3_a}

Let's also assume that $\mathfrak{B}$ is a line of length $k$. Since a this structure doesn't contain a cycle, we can affirm that the aciclicity query is not respected by it.
\input{structures/ex1_5_3_b}
\end{proof}
