...
\begin{proof}
	...
	\noindent\emph{Round 1}, \emph{Round 2}:
	...
	It's easy to see that all the mentioned scenarios are possible, since both structures contain three connected nodes (e.g., $v_1$, $v_2$, $v_3$ in $\mathfrak{G}_1$, and $u_1$, $u_2$, $u_3$ in $\mathfrak{G}_2$), two connected nodes and a third one disconnected from the first two (e.g., $v_1$, $v_2$, $v_4$ in $\mathfrak{G}_1$, and $u_1$, $u_2$, $w_1$ in $\mathfrak{G}_2$), and three disconnected nodes (e.g., $v_1$, $v_3$, $v_5$ in $\mathfrak{G}_1$, and $u_1$, $u_3$, $w_1$ in $\mathfrak{G}_2$).
	...
	\noindent\emph{Round 4}:
	
	Spoiler chooses three connected nodes in structure $\mathfrak{G}_2$ in the first three rounds (e.g., $u_1, u_2, u_3$), in the fourth round he chooses the fourth node remaining in the cycle (i.e., $u_4$), effectively creating a cycle of length 4. Therefore, Spoiler wins, since there is no 4-cycle in structure $\mathfrak{G}_1$.
	
	\noindent\emph{Conclusion}:
	
	Similarly to the $n = 1$ case, we saw that Discriminator can win all 3-rounds games on these structures, while he has no winning condition on some of the 4-rounds games. Therefore, $\mathfrak{G}_1$ and $\mathfrak{G}_2$ cannot be distinguished by a sentence of a quantifier-rank $3$ or less (i.e., $\mathfrak{G}_1 \equiv_3 \mathfrak{G}_2$), and there is a sentence of quantifier-rank $4$ with which we can distinguish the two structure (i.e., $\mathfrak{G}_1 \not\equiv_4 \mathfrak{G}_2$).
	...
	\proofpart{Bonus.}
	To prove that the connectivity query is not expressible in the language of graphs over finite graphs, we need to show that, for every $k \in \mathbb{N}$, there exist two structures, respectively:
	
	\begin{itemize}
		\item $\mathfrak{G}_1$, a connected graph,
		\item $\mathfrak{G}_2$, a disconnected graph
	\end{itemize}
	such that $\mathfrak{G}_1 \equiv_k \mathfrak{G}_2$.
	
	Intuitively, in order to build such structures, one could build $\mathfrak{G}_1$ and $\mathfrak{G}_2$ such that their number of nodes is very big compared to the fixed number of moves for the game. For the first structure, we can consider a (very big) cycle, and for the second we can consider a structure made by two disjoint connected graphs. In particular, we can generalize the first two cases of the exercise: for each $k \in \mathbb{N}$, we can consider $\mathfrak{G}_1$ to be the cycle of length $4k$, and $\mathfrak{G}_2$ to be the disjoint structure composed by two cycles of length $2k$.
	
	To show that the two structures are indistinguishable, we need to show that the Duplicator always has a winning strategy over all games $G_k$, i.e. can win the first $k$ rounds of every game $G_k$.
...
\noindent\emph{Round 1}, \emph{Round 2} and \emph{Round 3}:
